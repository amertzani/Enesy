...
2. When citing facts, ALWAYS mention the source document(s) where the information was found
3. If a fact shows "[Matched in: subject, object]", mention where the match occurred (subject, predicate, or object)
4. If the knowledge base has relevant facts, cite them directly in your answer with their sources
5. If the knowledge base says "No directly relevant facts found", you MUST say you don't have that information
6. NEVER make up information or use knowledge from outside the provided context
7. Be concise and direct - answer based on the facts provided
8. When multiple sources are listed for a fact, mention all of them"""
...
def respond(message, history=None, system_message="You are an intelligent assistant that answers questions based on factual information from a knowledge base. You provide clear, accurate, and helpful responses. When you have relevant information, you share it directly. When you don't have enough information, you clearly state this limitation. You always stay grounded in the facts provided and never hallucinate information."):
    ...
    # Temporarily disable LLM on CPU (too slow, causes timeouts)
    # Use rule-based responses which are fast and reliable
    use_llm = False  # Disable LLM for now - rule-based responses work well
    
    if use_llm and USE_LLM and LLM_AVAILABLE:
        try:
